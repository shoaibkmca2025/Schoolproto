...
export default function ReceiptPreview() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {/* Printable Area */}
      <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-200 min-h-[794px] print-container" ref={componentRef}>
        {/* Header */}
        <div className="flex justify-between items-start mb-6">
          <div className="flex gap-4">
            <div className="size-16 rounded-lg overflow-hidden border border-slate-100 shadow-sm bg-white">
              <img src={logoBase64} alt="Yashodai Logo" className="w-full h-full object-contain p-1" referrerPolicy="no-referrer" />
            </div>
            <div>
              <h2 className="text-xl font-bold text-slate-900 leading-tight">Yashodai Play School & Nursery</h2>
              <p className="text-blue-600 font-bold text-xs mb-1">Nashik Branch</p>
          ...
          <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 text-right min-w-[140px]">
            <p className="text-[8px] text-slate-400 uppercase font-bold tracking-wider mb-0.5">Receipt No</p>
            <p className="text-sm font-bold text-slate-900 mb-2">#{payment.receiptNo}</p>
            <p className="text-[8px] text-slate-400 uppercase font-bold tracking-wider mb-0.5">Date</p>
            <p className="text-xs font-bold text-slate-900">{format(new Date(payment.date), 'dd MMM, yyyy')}</p>
          </div>
        </div>
        ...
        <div className="space-y-3 mb-6">
          <h3 className="text-[10px] font-bold text-blue-600 uppercase tracking-widest flex items-center gap-1.5">
            <User size={12} />
            Student Details
          </h3>
          <div className="bg-slate-50 p-4 rounded-lg grid grid-cols-2 gap-y-3 gap-x-8">
            <div className="flex justify-between border-b border-slate-200 pb-1">
              <span className="text-[11px] text-slate-500">Student Name</span>
            ...
            <div className="flex justify-between border-b border-slate-200 pb-1">
              ...
              <span className="text-[11px] font-bold text-slate-900">{admission.admissionNo}</span>
            </div>
            <div className="flex justify-between border-b border-slate-200 pb-1">
              <span className="text-[11px] text-slate-500">Father's Name</span>
              <span className="text-[11px] font-bold text-slate-900">{student.fatherName}</span>
            </div>
            <div className="flex justify-between border-b border-slate-200 pb-1">
              <span className="text-[11px] text-slate-500">Class</span>
              <span className="text-[11px] font-bold text-slate-900">{student.class}</span>
            </div>
            <div className="flex justify-between border-b border-slate-200 pb-1">
              <span className="text-[11px] text-slate-500">Mother's Name</span>
              <span className="text-[11px] font-bold text-slate-900">{student.motherName}</span>
            </div>
            <div className="flex justify-between border-b border-slate-200 pb-1">
              <span className="text-[11px] text-slate-500">Academic Year</span>
              <span className="text-[11px] font-bold text-slate-900">{admission.academicYear}</span>
            </div>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-100">
                <tr>
                  <td className="px-4 py-3">
                    <p className="text-[11px] font-bold text-slate-900">Installment #{payment.installmentNumber}</p>
                    <p className="text-[9px] text-slate-500">{admission.installmentType}</p>
                  </td>
                  <td className="px-4 py-3 text-[11px] font-medium text-slate-700">{payment.paymentMode}</td>
                  <td className="px-4 py-3 text-[11px] font-medium text-slate-700">{payment.transactionId || '-'}</td>
                  <td className="px-4 py-3 text-right text-[11px] font-bold text-slate-900">₹ {payment.amount.toLocaleString()}.00</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="flex justify-between items-start gap-6 mb-6">
          <div className="flex-1 bg-blue-50 p-4 rounded-lg border border-blue-100">
            <p className="text-[8px] text-blue-600 uppercase font-bold tracking-wider mb-1">Amount in Words</p>
            <p className="text-xs font-bold text-slate-900 italic leading-tight">Rupees {numberToWords(payment.amount)}</p>
          </div>
          <div className="w-[200px] space-y-2">
            ...
            <div className="bg-blue-600 p-3 rounded-lg text-white flex justify-between items-center">
              <span className="text-[9px] font-bold uppercase">Balance</span>
              <span className="text-base font-bold">₹ {remainingBalance.toLocaleString()}.00</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-auto pt-6 border-t border-slate-100 flex justify-between items-end">
          <div className="max-w-[250px] space-y-1">
            <p className="text-[8px] font-bold text-slate-400 uppercase">Terms & Conditions:</p>
            <ul className="text-[8px] text-slate-400 space-y-0.5 list-disc pl-3">
          ...
          <div className="text-center">
            ...
            <p className="text-[8px] text-slate-500">School Office Seal</p>
          </div>
        </div>

        <div className="mt-6 text-center">
          <p className="text-[8px] text-slate-400 mb-0.5">Thank you for choosing Yashodai Play School.</p>
          <p className="text-[8px] font-bold text-blue-600">www.yashodaischool.edu.in</p>
        </div>
      </div>
    </div>
  );
